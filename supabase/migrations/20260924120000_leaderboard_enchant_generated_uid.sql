-- Generated leaderboard identities expose only a stable short identifier.
-- The public DTO marks them so the client can render the distinct display font.
...
as $$
  select
    coalesce(
      ...
      substring(replace(a.user_id::text, '-', ''), 1, 8)
    ) as nickname,
