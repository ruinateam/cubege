-- Every submitted attempt participates in the public top. A non-approved
-- nickname is replaced by a stable generated pseudonym. One grouped row keeps
-- each player's best score and total submitted-attempt count.
...
as $$
  select
    coalesce(
      ...
      (case when substring(replace(a.user_id::text, '-', ''), 1, 1) between '0' and '7'
        then 'Steve_'
        else 'Alex_'
      end) || substring(replace(a.user_id::text, '-', ''), 1, 8)
    ) as nickname,
    coalesce(exists (
      select 1
      from auth.users u
