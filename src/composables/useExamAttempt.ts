...
import { ensureAnonymousSession, getAttemptDeadline, getAttemptResults, getVariantQuestions, saveAnswer, startAttempt, submitAttempt, type AttemptReview, type ExamVariant } from '@/lib/supabase'
...
export function useExamAttempt() {
  ...
  function reset() { answers.value = {}; currentIndex.value = 0; secondsLeft.value = 0; activeAttemptId.value = null; expiresAt.value = null; serverScore.value = { primary: 0, secondary: 0 }; serverReview.value = {}; localStorage.removeItem(draftKey()) }
  ...
  return { questions, question, currentIndex, answers, secondsLeft, formattedTime, savedAt, authMessage, answeredCount, maxPrimary, shortCount, primaryScore, secondaryScore, rank, activeTitle, activeAttemptId, serverReview, isSubmitting, isLoading, start, submit, setQuestion, reset }
}
